...
void main()
{
    ...
    // Temel ışık yönü hesaplama
    vec3 N = normalize(vNormal);
    vec3 L = normalize(vec3(0.0, 0.0, 1.0));
    
    // Diffuse
    float lambert = dot(N, L);
    diff = max(lambert, 0.0) * 5.0; // Çok daha güçlü diffuse etkisi
    
    // Specular
    vec3 V = normalize(-vPosition);
    vec3 H = normalize(L + V);
    float NdotH = max(dot(N, H), 0.0);
    spec = pow(NdotH, 50.0) * 2.0; // Daha güçlü ve keskin specular
    ...
    if (isLightSource) {
        ...
        gl_FragColor = texture2D(tex, vTexCoord) * vec4(ambient + diff + spec);
    }
}
`;
