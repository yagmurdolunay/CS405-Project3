...
void main()
{
	...
	// Diffuse ış��k hesaplama - daha güçlü bir etki için 0.8 ile çarpıyoruz
	diff = 0.8 * max(dot(normal, lightdir), 0.0);

	// Specular ışık hesaplama - daha belirgin highlight için 0.5 ile çarpıyoruz
	vec3 viewDir = normalize(-vPosition);
	vec3 halfwayDir = normalize(lightdir + viewDir);
	spec = 0.5 * pow(max(dot(normal, halfwayDir), 0.0), phongExp);
