...
void main()
{
    ...
    // Debug için normal kontrolü
    if(length(normal) < 0.1) {
        diff = 1.0; // Normal çok kısa ise
    } else {
        // Diffuse hesaplama
    ...
    // View vector hesaplama
    vec3 viewDir = normalize(-vPosition); // Kamera origin'de (0,0,0)
    
    // Specular hesaplama
    vec3 reflectDir = reflect(-lightdir, normal);
    float RdotV = max(dot(reflectDir, viewDir), 0.0);
    spec = pow(RdotV, phongExp);
