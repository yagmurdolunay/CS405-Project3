...
void main()
{
	...
	// PLEASE DO NOT CHANGE ANYTHING ABOVE !!!
	 // Diffuse - çok daha güçlü bir etki için 3.0
    diff = max(dot(normal, lightdir), 0.0) * 3.0;

    // Specular - daha keskin highlight için 128.0
    vec3 viewDir = normalize(vec3(0.0, 0.0, 5.0) - fragPos);
    vec3 reflectDir = reflect(-lightdir, normal);
    spec = pow(max(dot(viewDir, reflectDir), 0.0), 128.0) * 0.6;
